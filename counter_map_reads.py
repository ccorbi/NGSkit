...
__about__ = ''' This is a small script to count and rank reads from a fasta file. This script is agnostic, do not compare or map
             the reads with any source or library'''


def load_blastout(filename):
    df = pd.read_csv(filename,delim_whitespace=True,names=['queryId', 'subjectId', 'percIdentity', 'alnLength', 'mismatchCount', 'gapOpenCount', 'queryStart', 'queryEnd', 'subjectStart', 'subjectEnd', 'eVal', 'bitScore'])
    return df


def load_bowtieout(filename):
    df = pd.read_csv(filename,names=['queryId','strand', 'subjectId','off','seq','otheInstances','quality','mismatchDescriptor'],sep='\t')
    return df


def filter_blast(df,iden=100.0,l=54,m=0):
    df =   df[(df['percIdentity']>=100.0)&
                       (df['alnLength']==l)&
                       (df['mismatchCount']<=m)]
    return df
...
def from_bowtie_generate_ranking_file(df,outputname,**kargs):
    ...
    q.to_csv(outputname,index=False,columns=['subjectId','aa','Counts'])
    return q
...
def get_options():
    ...
    parser.add_argument('--blast', help='Count sequences from a blast output', dest = 'blast', default = False,
                      action = 'store_true')

    parser.add_argument('--bowtie', help='Count Sequences from bowtie output', dest = 'bowtie', default = False,
                      action = 'store_true')
    ...
    parser.add_argument('-i','--input_file', action="store", dest="input_file",
        required=True,help='file with mapped sequences bowtie/blast')



    parser.add_argument('-s','--library_size', action="store", dest="library_size",
        help='Size of the library',type=int)
...
if __name__ == '__main__':
    ...
    if opts.bowtie:
        ...
        if opts.output:
            ...
            print('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}'.format(opts.input_file,raw_seqs,
                                                            filter_seqs,
                                                            unique_seqs,
                                                            coverage,
                                                            s[u'mean'], s[u'std'], s[u'min'],s[u'25%'],s[u'50%'],s[u'75%'],s[u'max']))


        else:
            #print summary
            print('Raw Mapped READS\tFiltered Mapped READS\tUnique Reads\tlibrary Coverage\t')
            print('{}\t{}\t{}\t{}'.format(raw_seqs,filter_seqs,unique_seqs,coverage))